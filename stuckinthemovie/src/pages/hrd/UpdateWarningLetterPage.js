...
function UpdateWarningLetterPage() {
  ...
  function updateLinkHandler(userid, letterid) {
    const link = `${storageURL}warning-letter%2F${userid}%2F${fileName.current}?alt=media`;
    const updatedate = format(new Date(), "dd/MM/yyyy").toString();
    fetch(`${baseURL}warning-letter/${userid}/${letterid}.json`, {
      method: "PATCH",
      body: JSON.stringify({
        letter: link,
...
export default UpdateWarningLetterPage;
