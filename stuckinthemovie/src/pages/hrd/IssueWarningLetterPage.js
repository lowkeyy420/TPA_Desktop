...
function IssueWarningLetterPage() {
  const [isLoading, setLoading] = useState(true);
  const [loadedEmployees, setLoadedEmployees] = useState([]);
  const [loadedWarningLetter, setLoadedWarningLetter] = useState([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      //all warning letter
      const letterRes = await fetch(`${baseURL}warning-letter.json`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const filteredWarningLetter = loadedWarningLetter.filter(
    (employee) =>
      employee.name &&
  ...
  function fileUploadHandler(userid) {
    ...
    fetch(
      `${storageURL}warning-letter%2F${userid}%2F${fileName.current}?alt=media`,
      {
        method: "POST",
        body: formData,
      }
  ...
  function createWarningLetter(userid) {
    const issuedDate = format(new Date(), "dd/MM/yyyy").toString();
    const issuer = authCtx.user;

    const letterURL = `${storageURL}warning-letter%2F${userid}%2F${fileName.current}?alt=media`;
    fetch(`${baseURL}warning-letter/${userid}.json`, {
      method: "POST",
      body: JSON.stringify({
      ...
      if (res.ok) {
        alert("Successfully added new warning letter");
      } else {
        alert("Error occured");
      }
  ...
  return (
    <div className={classes.datatable}>
      <div>
        <h2>Issued Warning Letters</h2>
        <DataTable
          columns={columns}
          data={filteredWarningLetter}
          keyField={""}
...
export default IssueWarningLetterPage;
