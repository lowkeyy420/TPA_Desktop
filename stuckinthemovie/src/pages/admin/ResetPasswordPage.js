...
function ResetPasswordPage() {
  ...
  useEffect(() => {
    function resetPasswordHandler(data) {
      const key = process.env.REACT_APP_FIREBASE_PROJECTKEY;
      const resetURL = `https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=${key}`;
      ...
      .then((data) => {
        ...
        for (const key in data) {
          const employee = {
            ...
            ...data[key],
            action: (
              <button onClick={() => resetPasswordHandler(data[key])}>
                Reset Password
              </button>
            ),
          };
